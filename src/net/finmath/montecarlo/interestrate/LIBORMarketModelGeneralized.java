...
/**
 * Implements a normal LIBOR market model with some drift approximation methods.
 * <br>
 * The class implements different measure(drift) / numeraire pairs (terminal and spot).
 * <br>
 * The class specifies a LIBOR market model in its normal formulation, that is
 * <i>L<sub>j</sub> = Y<sub>j</sub> </i> where
 * <br>
 ...
 * <br>
 * Note that &lambda; may still depend on <i>L</i>, hence generating a log-normal model.
 * 
 ...
 * @version 1.1
 */
public class LIBORMarketModelGeneralized extends AbstractModel implements LIBORMarketModelInterface {
	...
	private boolean isLognormal = false;
	 ...
	 */
	public LIBORMarketModelGeneralized(
	...
	public RandomVariableInterface[] getInitialState() {
		...
		for(int timeIndex=0; timeIndex<liborPeriodDiscretization.getNumberOfTimeSteps(); timeIndex++) {
			...
			liborInitialStates[timeIndex] = isLognormal ? Math.log(rate) : rate;
		}
	 ...
	 * 
	 * @see net.finmath.montecarlo.interestrate.LIBORMarketModelGeneralized#getNumeraire(int) The calculation of the drift is consistent with the calculation of the numeraire in <code>getNumeraire</code>.
	 * 
	...
	public RandomVariableInterface[] getDrift(int timeIndex, RandomVariableInterface[] realizationAtTimeIndex, RandomVariableInterface[] realizationPredictor) {
		...
		for(int componentIndex=firstLiborIndex; componentIndex<getNumberOfComponents(); componentIndex++) {
			...
			RandomVariableInterface oneStepMeasureTransform = (new RandomVariable(1.0)).discount(libor, periodLength).mult(periodLength);

    		if(isLognormal) oneStepMeasureTransform = oneStepMeasureTransform.mult(libor);
	...
	public RandomVariableInterface applyStateSpaceTransform(int componentIndex, RandomVariableInterface randomVariable) {
		if(isLognormal)	return randomVariable.exp();
	...
	@Override
	public Object clone() {
		return new LIBORMarketModelGeneralized(liborPeriodDiscretization, forwardRateCurve, covarianceModel);
	}
	 ...
	 */
	@Override
	public LIBORMarketModelGeneralized getCloneWithModifiedCovarianceModel(AbstractLIBORCovarianceModel covarianceModel) {
		LIBORMarketModelGeneralized model = (LIBORMarketModelGeneralized)this.clone();
		model.covarianceModel = covarianceModel;
		return model;
	}

	@Override
	public LIBORMarketModelGeneralized getCloneWithModifiedData(Map<String, Object> dataModified) throws CalculationException {
		...
		if(swaptionMarketData == null) {
			...
			return new LIBORMarketModelGeneralized(liborPeriodDiscretization, forwardRateCurve, covarianceModel, swaptionMarketData);
