...
public class StochasticLevenbergMarquardtTestInverse {
	...
	public RandomVariable[][] multiply(final RandomVariable[][] matrixA, final RandomVariable[][] matrixB){
		final RandomVariable[][] AB = new RandomVariable[matrixA.length][matrixB.length];
		RandomVariable ABproduct;
		for(int i=0;i<matrixA.length;i++){
			for(int j=0; j<matrixB.length; j++) {
				AB[i][j] = new RandomVariableFromDoubleArray(0.0);
				for(int k=0;k<matrixB.length;k++) {
					if(matrixA[i][k]==null || matrixB[k][j]==null) {ABproduct = new RandomVariableFromDoubleArray(0.0);}
