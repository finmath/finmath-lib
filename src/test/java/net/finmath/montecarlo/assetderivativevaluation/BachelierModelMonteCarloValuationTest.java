...
public class BachelierModelMonteCarloValuationTest {
	...
	{
		final BachelierModelMonteCarloValuationTest pricingTest = new BachelierModelMonteCarloValuationTest(new RandomVariableFromArrayFactory(true /* isUseDoublePrecisionFloatingPointImplementation */));
	...
	public BachelierModelMonteCarloValuationTest(final RandomVariableFactory randomVariableFactory) {
		super();
	...
	public AssetModelMonteCarloSimulationModel getModel()
	{
		...
		if(model == null) {
			...
			// Create the model
			final ProcessModel blackScholesModel = new BachelierModel(initialValue, riskFreeRate, volatility);

			// Create a corresponding MC process
			final MonteCarloProcessFromProcessModel process = new EulerSchemeFromProcessModel(blackScholesModel, new BrownianMotionLazyInit(timeDiscretization, 1 /* numberOfFactors */, numberOfPaths, seed, randomVariableFactory));
	...
	public void testEuropeanCall() throws CalculationException
	{
		...
		final double volBachelier = volatility / payoffUnit;
	...
	public void testEuropeanCallVega() throws CalculationException
	{
		...
		for(double optionStrike = 0.60; optionStrike < 1.50; optionStrike += 0.05) {
			...
			final double volBachelierUp = (volatility+shift) / payoffUnit;
			final double volBachelierDown = (volatility-shift) / payoffUnit;
			// Calculate the finite difference of the analytic value
