...
public class AnalyticFormulas {
    ...
    public static double constantElasticityOfVarianceOptionValue(
            ...
            double optionStrike,
            boolean isCall) {
        double kappa = 2 * riskFreeRate / (Math.pow(volatility, 2) * (1 - exponent) * (Math.exp(2 * riskFreeRate * (1 - exponent) * optionMaturity) - 1));
        double z = 2 + 1 / (1 - exponent);
        double y = kappa * Math.pow(optionStrike, 2 * (1 - exponent));
        double x = kappa * Math.pow(initialStockValue, 2 * (1 - exponent)) * Math.exp(2 * riskFreeRate * (1 - exponent) * optionMaturity);
        NonCentralChiSquaredDistribution P1 = new NonCentralChiSquaredDistribution(z, x);
        NonCentralChiSquaredDistribution P2 = new NonCentralChiSquaredDistribution(z - 2, y);
        if (isCall) {
