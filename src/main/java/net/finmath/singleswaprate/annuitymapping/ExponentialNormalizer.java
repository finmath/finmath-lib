...
public class ExponentialNormalizer implements NormalizingFunction {
	...
	public ExponentialNormalizer(Schedule fixSchedule, Schedule floatSchedule, String discountCurveName, String forwardCurveName, String volatilityCubeName, VolatilityCubeModel model) {
		...
		NormalizingDummyProduct unscaledDummy = new NormalizingDummyProduct(fixSchedule, floatSchedule, discountCurveName, forwardCurveName, volatilityCubeName, new ExponentialNormalizer(initialSwapRate, 1));
	...
	public double getValue(double swapRate) {
		double exponent = - swapRate * swapRate / initialSwapRate / initialSwapRate;
		return scale * Math.exp(exponent);
	}

	@Override
	public double getFirstDerivative(double swapRate) {
		double factor = -2 * swapRate / initialSwapRate / initialSwapRate;
		return factor * getValue(swapRate);
	}

	@Override
	public double getSecondDerivative(double swapRate) {
		double factor = 2 / initialSwapRate / initialSwapRate;
