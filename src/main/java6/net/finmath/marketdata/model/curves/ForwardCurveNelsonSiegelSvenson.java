...
public class ForwardCurveNelsonSiegelSvenson extends AbstractCurve implements Serializable, ForwardCurveInterface {
	 ...
	 * @param paymentOffsetCode The payment offset code, like 3M, 6M, 12M, etc., used incalculating forwards from disocunt factors.
	 ...
	 */
	public ForwardCurveNelsonSiegelSvenson(String name, Calendar referenceDate, String paymentOffsetCode, BusinessdayCalendarInterface paymentBusinessdayCalendar, BusinessdayCalendarInterface.DateRollConvention paymentDateRollConvention, DayCountConventionInterface daycountConvention, double[] parameter) {
		...
		discountCurve = new DiscountCurveNelsonSiegelSvenson(name, referenceDate, parameter);
	}
	...
	public double getForward(AnalyticModelInterface model, double fixingTime, double paymentOffset) {
		...
		return (discountCurve.getDiscountFactor(model, getCurveTime(fixingTime)) / discountCurve.getDiscountFactor(model, getCurveTime(fixingTime + paymentOffset)) - 1.0) / daycountFraction;
	}

	public double getCurveTime(double time) {
		// Create model date from model time
